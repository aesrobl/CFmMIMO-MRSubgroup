...
for g = 1:length(G_Values)
    ...
    for n = 1:nbrOfSetups
        ...
        [SE_MR_enhanced_multi(:,n)] = functionComputeSE_downlink_multicast_MR_Enhanced(HhatG,HMulti,DMulti,AMulti,tau_c,tau_p,nbrOfRealizations,N,sum(K),G,L,rho_tot,spatial_sub);
        [ASE_MR_enhanced_multi(n)] = sum(SE_MR_enhanced_multi(:,n));
    end
    
    %Prepare to save simulation results
    results_filename = [num2str(L) 'x' num2str(N) '-' num2str(Cluster) 'x' num2str(Kc) '-MR-enhanced-multi' num2str(G) '.mat'];
    save(results_filename,'SE_MR_enhanced_multi','ASE_MR_enhanced_multi');
    time = toc;
