...
for g = 1:length(G_Values)
    ...
    for n = 1:nbrOfSetups
        ...
        [gainOverNoisedB,R,pilotIndexMulti,spatial_sub,DMulti] = generateSetupMulticast(L,N,K,G,tau_p,1,n); %Cambiar por el n y mirar para tener siempre el mismo escenario y comparar los distintos precodificadores       
        ...
        [SE_MR_multi(:,n)] = functionComputeSE_downlink_multicast_MR(HhatG,HMulti,DMulti,AMulti,BMulti,tau_c,tau_p,nbrOfRealizations,N,sum(K),G,L,rho_tot,spatial_sub);
        [ASE_MR_multi(n)] = sum(SE_MR_multi(:,n));
    end
    
    results_filename = [num2str(L) 'x' num2str(N) '-' num2str(Cluster) 'x' num2str(Kc) '-MR-multi' num2str(G) '.mat'];
    save(results_filename,'SE_MR_multi','ASE_MR_multi');
    time = toc;
