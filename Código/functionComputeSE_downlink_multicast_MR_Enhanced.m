function [SE_MR] = functionComputeSE_downlink_multicast_MR_Enhanced(HhatG,H,D,A,tau_c,tau_p,nbrOfRealizations,N,K,G,L,rho_tot,spatial_sub)
...
signal_MR = zeros(K,1);
interf_MR = zeros(K,1);
scaling_MR_Enhanced = zeros(L,G);
nu = 0.5;

%% Compute scaling factors for precoding
%Go through all realizations to calculate avarage from channel realizations
for n = 1:nbrOfRealizations
    %Go through all subgroups
    for g = 1:G
        %Determine the set of serving APs for each group and initiate the
        %matrix for the channel estimations
        servingAPs = find(D(:,g)==1);
        La = length(servingAPs);        
        HhatGallj_active = zeros(N*La,G);
        % Go through each AP from subgroup g to calculate the channel
        % estimation from every serving AP
        for l = 1:La
            HhatGallj_active((l-1)*N+1:l*N,:) = reshape(HhatG((servingAPs(l)-1)*N+1:servingAPs(l)*N,n,:),[N G]);
            %Compute scaling factor using channel realizations by Monte
            %Carlo methods for every AP that serves the subgroup g
            scaling_MR_Enhanced(l,g) = scaling_MR_Enhanced(l,g) + (1/norm(HhatGallj_active((l-1)*N+1:l*N,g))^2)/nbrOfRealizations;
        end
    end
end
for l = 1:L
    ...
    for ind = 1:length(servedSubgroups)
        ...
        %No lo necesitamos por haberlo calculado en el paso anterior con la
        %esperanza pasando por todas las realizaciones del canal no?
        %Compute scaling factor using the spatial correlation matrix of the
        %channel estimate       
        %scaling_MR_Enhanced(l,servedSubgroups(ind)) = trace(B(:,:,l,servedSubgroups(ind)));      
    end    
end
...
for n = 1:nbrOfRealizations   
    ...
    for g = 1:G        
        ...
        for l = 1:La
            ...
            HhatGallj_active((l-1)*N+1:l*N,:) = reshape(HhatG((servingAPs(l)-1)*N+1:servingAPs(l)*N,n,:),[N G]);
            %Compute MR precoding for Enhanced Case
            w((l-1)*N+1:l*N) = HhatGallj_active((l-1)*N+1:l*N,g)/(norm(HhatGallj_active((l-1)*N+1:l*N,g)).^2);
            %Apply power allocation
            % COMENTAR por qué usamos scaling_MR_Enhanced(l,g) en lugar de scaling_MR_Enhanced(servingAPs(l),g)
            w((l-1)*N+1:l*N) = w((l-1)*N+1:l*N)*sqrt(rho_dist(servingAPs(l),g)/(scaling_MR_Enhanced(l,g)));     
        end
        w = w.';
