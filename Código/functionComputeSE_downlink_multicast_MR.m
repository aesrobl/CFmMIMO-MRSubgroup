function [SE_MR] = functionComputeSE_downlink_multicast_MR(HhatG,H,D,A,B,tau_c,tau_p,nbrOfRealizations,N,K,G,L,rho_tot,spatial_sub)
...
for l = 1:L
    ...
    for ind = 1:length(servedSubgroups)
        ...
        %channel estimate       
        scaling_MR(l,servedSubgroups(ind)) = trace(B(:,:,l,servedSubgroups(ind)));      
    end    
end
...
for n = 1:nbrOfRealizations   
    ...
    for g = 1:G        
        ...
        for l = 1:La
            ...
            w((l-1)*N+1:l*N) = HhatGallj_active((l-1)*N+1:l*N,g);
            %Apply power allocation
            w((l-1)*N+1:l*N) = w((l-1)*N+1:l*N)*sqrt(rho_dist(servingAPs(l),g)/(scaling_MR(servingAPs(l),g)));     
        end
        w = w.';
